 ...
 * 适用于只有两张图片 并且一张是底图 一张是上面的图 上图里面包含了勾和底图
 */

public class My2CheckBox extends TImage {
    private boolean checked = false;
    private TextureRegionDrawable gou, di;
    ...
    public My2CheckBoxGroup buttonGroup;

    public My2CheckBox(final TextureRegion di, final TextureRegion gou) {
        super(di);
        this.gou = new TextureRegionDrawable(gou);
        this.di = new TextureRegionDrawable(di);

        checkedListeners = new Array<>();

        nativeLabel = new NativeLabel("", TGame.getDefaultFont());
    ...
    public void setChecked(boolean checked) {
        ...
        if (this.checked) {
            setDrawable(My2CheckBox.this.gou);
        } else {
            setDrawable(My2CheckBox.this.di);
        }
    }

    public My2CheckBoxGroup getButtonGroup() {
        return buttonGroup;
    }
