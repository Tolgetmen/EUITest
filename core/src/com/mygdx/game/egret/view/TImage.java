 ...
 * 创建时间：2016/6/22 15:32
 * 修改人：Administrator
 * 修改时间：2016/6/22 15:32
 * 修改备注：
 */
public class TImage extends Image {
    public boolean doudou = false;
    ...
    public TImage isButton(final TClickListener clickListener) {
        origonCenter();
    ...
    public TImage isNoButton(final TClickListener clickListener) {
        origonCenter();
        addListener(new ClickListener() {
            @Override
            public void clicked(InputEvent event, float x, float y) {
    ...
    public TImage isColorButton(final TClickListener clickListener) {
        addListener(new ClickListener() {
            @Override
     ...
     * @param midMenu_bg
     */
    public TImage alignCenter(TImage midMenu_bg) {
        setY(midMenu_bg.centerY() - getHeight() / 2f);
    ...
    /*public static TImage makeAlphaImage() {
        Pixmap pixmap = new Pixmap((int) MyGame.worldWidth, (int) MyGame.worldHeight, Pixmap.Format.RGBA8888);
    ...
    /*public static TImage makeImage(Color color) {
        Pixmap pixmap = new Pixmap((int) MyGame.worldWidth, (int) MyGame.worldHeight, Pixmap.Format.RGBA8888);
    ...
    public static TImage loadInerImg(String name) {
        TImage bgImg = new TImage(getTexture(name));
        return bgImg;
    }

    public static TImage loadInerNoImg(String name) {
        TImage bgImg = new TImage(getNoTexture(name));
        return bgImg;
    }

    public static TImage loadInerImgFlip(String name) {
        TextureRegion region = new TextureRegion(getTexture(name));
        region.flip(true, false);
        TImage bgImg = new TImage(region);
        return bgImg;
    }

    public static Drawable loadInerDrawable(String name) {
        return new TextureRegionDrawable(new TextureRegion(getTexture(name)));
    }

    public static Drawable loadInerNoDrawable(String name) {
        return new TextureRegionDrawable(new TextureRegion(getNoTexture(name)));
    }

    public static Drawable loadInerFlipDrawable(String name) {
        TextureRegion region = new TextureRegion(getTexture(name));
        region.flip(true, false);
        return new TextureRegionDrawable(region);
    }

    public static TImage loadInerImg(String name, boolean flipX, boolean flipY) {
        TextureRegion textureRegion = new TextureRegion(getTexture(name));
        textureRegion.flip(flipX, flipY);
        return new TImage(textureRegion);
    }

    public static TImage loadInerNoImg(String name, boolean flipX, boolean flipY) {
        TextureRegion textureRegion = new TextureRegion(getNoTexture(name));
        textureRegion.flip(flipX, flipY);
        return new TImage(textureRegion);
    }
    ...
    public static Texture getNoTexture(String path) {
        Texture texture = new Texture(path);
        return texture;
    }
